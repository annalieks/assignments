...
class Display:
    def draw_window(self, win, grid, display_info, pacman, ghosts, pygame, score, path, nodes):
        ...
        for i in range(len(ghosts)):
            win.blit(self.get_ghost_direction_image(i, ghosts, display_info),
                     (display_info.ghost_x[i], display_info.ghost_y[i]))
    ...
    def draw_game_over(self, win, display_info, pygame, if_win):
        win.fill((17, 14, 61))
        win.blit(display_info.game_over_img,
                 ((display_info.display_width - 331) / 2, (display_info.display_height - 137) / 2))
